...
if (( stale == 1 )); then
    printf '%s #[fg=colour245]│#[default] %s #[fg=colour245]│#[default] %s #[fg=colour250,bg=colour240] stale %sm #[default]\n' \
        "$five_segment" "$seven_segment" "$tools_segment" "$((cache_age / 60))"
    exit 0
fi
